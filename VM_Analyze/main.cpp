...
SEG_MAP segs[] = {//将内存dump 出来
	//base			size			file name  需求确定内存没有改变... dump的不能先运行一段时间然后dump
...
int main(int argc, char **argv, char **envp)
{
	...
	while (1){
		...
		case 0x4010AF://读取值到buf
		{
			...
			regs.regs.r_eip += 6;
			uc_reg_write(uc, UC_X86_REG_EIP, &regs.regs.r_eip);
			regs.regs.r_eax = 1;//读取成功
			uc_reg_write(uc, UC_X86_REG_EAX, &regs.regs.r_eax);
			regs.regs.r_edx = 1;//读取成功
			uc_reg_write(uc, UC_X86_REG_EDX	, &regs.regs.r_edx);
			continue;
		}
		case 0x47BB92://strcmp
		{
			...
			uc_reg_write(uc, UC_X86_REG_ESP, &regs.regs.r_esp);
			regs.regs.r_eax = 0;//相同 或者不同
			uc_reg_write(uc, UC_X86_REG_EAX, &regs.regs.r_eax);
			regs.regs.r_ecx = 0x402118;//输出地址
			uc_reg_write(uc, UC_X86_REG_ECX	, &regs.regs.r_ecx);
...
DEFAULT:
		default:
			{	/*DWORD val; 
				...
				if ( /*is_reg_tainted(X86_REG_EBP)*/ //过滤
					//regs.regs.r_ebp == INIT_EBP
